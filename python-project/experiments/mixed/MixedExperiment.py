...
class MixedExperiment(metaclass=ABCMeta):
    ...
    def run(self, run: int, n_folds: int, fold_log: bool):
        ...
        # Apply KDE
        results_path = "../../../results/run_" + str(run) + "/mixed/" + self.data_name + "/" + str(n_folds) + "_folds/KDE/"
        KDE.apply(train_datasets, self.var_types_string, test_datasets, n_folds, results_path, self.data_name, fold_log)

        # Apply MSPN
        results_path = "../../../results/run_" + str(run) + "/mixed/" + self.data_name + "/" + str(n_folds) + "_folds/MSPN/"
