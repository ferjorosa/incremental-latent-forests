...
public class BinG implements DiscreteMethod {
    ...
    public void runDiscrete(List<Tuple<DataOnMemory<DataInstance>, DataOnMemory<DataInstance>>> folds,
        ...
        /* Store models */
        storeDiscreteModels(models, "results/run_"+ run +"/discrete/"+ dataName+"/" + folds.size() + "_folds/binG", dataName, "binG");

        /* Show average time and score */
        showAverageScoreAndTime(scoresAndTimes);

        /* Store experiment results in a JSON file */
        storeResults(scoresAndTimes, "results/run_"+ run +"/discrete/"+ dataName+"/" + folds.size() + "_folds/binG", dataName + "_results_binG.json");
    }
