...
public class BLFM_GS {
    ...
    public Result learnModel(DataOnMemory<DataInstance> data,
        ...
        while(keepsImproving && currentSet.size() > 1) {
            ...
            while(keepExpansion) {
                Tuple<Variable, Result> partitionExpansion = expandPartition(partitionVariables, currentSet, bestResult.getPlateuStructure(), bestResult.getDag());
     ...
     * La cardinalidad del modelo se estima para cada caso.
     */
    private Tuple<Variable, Result> expandPartition(List<Variable> partitionVariables,
        ...
        if(bestModelScore > -Double.MAX_VALUE) {
            ...
            return new Tuple<>(bestVar, new Result(bestModel, bestModelScore, copyDAG, "ExpandPartition"));
        }

        return new Tuple<>(null, new Result(bestModel, bestModelScore, copyDAG, "ExpandPartition"));
    }
