...
public class VBEM {
    ...
    private void pyramidInitialization(DataOnMemory<DataInstance> data) {
        ...
        List<Tuple<CompoundVector, Double>> candidates = new ArrayList<>(config.initCandidates());
        candidates.add(new Tuple<>(this.plateuStructure.getLatentVariablesPosterior(), this.plateuStructure.getLogProbabilityOfEvidence()));
        this.plateuStructure.setEvidence(data.getList());
