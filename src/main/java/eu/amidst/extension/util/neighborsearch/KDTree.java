 ...
 *******************************************************************************/
package eu.amidst.extension.util.neighborsearch;

import eu.amidst.extension.util.distance.DistanceFunction;
import eu.amidst.extension.util.distance.EuclideanDistance;
import eu.amidst.extension.util.neighborsearch.util.HeapSelect;
 ...
 * @author Haifeng Li
 */
// TODO: Da muchos problemas el hecho de usar arrays con tipos parametrizados, estari bien probar a implementarlo con Colecciones
// TODO: Yo cambiaria todo eltema de Key y data. Es un lio y no aporta nada que yo vea. Creo que es para poder tenerlo en
// forma "raw" y con objetos wrapper. Es decir, que ordena con los datos pero el KDtree hacia fuera es de otras cosas de mas alto nivel
// TODO: Me ha lanzado varios StackOverflowErrors en Heart.arff, creo que es un bug causado por la recursividad
public class KDTree <E> implements NearestNeighborSearch<double[], E>, KNNSearch<double[], E>, RNNSearch<double[], E>, Serializable {
     ...
     */
    private boolean identicalExcluded = true; // TODO: No debe de estar funcionando correctamente porque me salen distancias 0.0
    /**
     * Distance function used inside the algorithm
     */
     ...
     * @param removeDuplicates KDTree removes duplicate nodes.
     */
    public KDTree(double[][] key, E[] data, DistanceFunction distanceFunction, boolean removeDuplicates) {
        if (key.length != data.length) {
            throw new IllegalArgumentException("The array size of keys and data are different.");
        }

        if(removeDuplicates) {
            Tuple<double[][], E[]> result = removeDuplicates(key, data);
            this.keys = result.getFirst();
            this.data = result.getSecond();
        } else {
            this.keys = key;
            this.data = data;
        }
    ...
    public KDTree(double[][] key, E[] data, boolean removeDuplicates) {
        this(key, data, new EuclideanDistance(), removeDuplicates);
    }
    ...
    private Tuple<double[][], E[]> removeDuplicates(double[][] key, E[] data) {

        Map<Double, Integer> uniqueKeysIndexes = new LinkedHashMap<>();
        List<E> uniqueData = new ArrayList<>();
        for(int i=0; i < key.length; i++) {
            double hashCode = Arrays.hashCode(key[i]);
            if(!uniqueKeysIndexes.containsKey(hashCode)) {
                uniqueKeysIndexes.put(hashCode, i);
                uniqueData.add(data[i]);
            }
        }

        double[][] uniqueKeys = new double[uniqueKeysIndexes.size()][];
        int index = 0;
        for(Integer uniqueKeyIndex: uniqueKeysIndexes.values()){
            uniqueKeys[index] = key[uniqueKeyIndex];
            index++;
        }

        return new Tuple<>(uniqueKeys, (E[]) uniqueData.toArray());
    }
     ...
     * tree node.
     *
     * It uses the squared distance to improve time performance.
     *
    ...
    private void search(double[] q, Node node, Neighbor<double[], E> neighbor) {
        if (node.isLeaf()) {
            ...
            for (int idx = node.index; idx < node.index + node.count; idx++) {
                ...
                double distance = this.distanceFunction.squareDistance(q, keys[index[idx]]);
    ...
    private void search(double[] q, Node node, double radius, List<Neighbor<double[], E>> neighbors) {
        if (node.isLeaf()) {
            ...
            for (int idx = node.index; idx < node.index + node.count; idx++) {
                ...
                double distance = this.distanceFunction.distance(q, keys[index[idx]]);
                if (distance <= radius) {
