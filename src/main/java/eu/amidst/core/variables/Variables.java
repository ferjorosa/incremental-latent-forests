...
public class Variables implements Iterable<Variable>, Serializable {
    ...
    public Variables(Variables variables) {
        ...
        for(Variable variable: variables) {
            Variable copyVar = variable.deepCopy();
            this.allVariables.add(copyVar);
            this.mapping.put(copyVar.getName(), copyVar);
        }
    }
    ...
    public Variable newIndicatorVariable(Variable var, double deltaValue) {
        ...
        this.mapping.put(newVarName, newVariable);
    ...
    public Variable getVariableByName(String name) {
        ...
        if(!this.mapping.containsKey(name))
            throw new IllegalArgumentException("Variable " + name + " is not part of the list of Variables");
    ...
    public void remove(Variable variable) {
        this.getListOfVariables().remove(variable);
        this.mapping.remove(variable.getName());
    }

    public void add(Variable variable) {
        if(this.mapping.containsKey(variable.getName()))
            throw new IllegalArgumentException("Variable already exists");
        for(Variable var: this.allVariables)
            if(var.getVarID() == variable.getVarID())
                throw new IllegalArgumentException("There is a variable with the same VarID");

        this.getListOfVariables().add(variable);
        this.mapping.put(variable.getName(), variable);
    }
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(allVariables, variables.allVariables) &&
                Objects.equals(mapping, variables.mapping);
    }

    @Override
    public int hashCode() {

        return Objects.hash(allVariables, mapping);
    }
    ...
    private static class VariableImplementation implements Variable, Serializable {
        ...
        /** Copy constructor. It is called by deepCopy() */
        public VariableImplementation(VariableImplementation variableImplementation) {
            this.name = variableImplementation.name;
            this.varID = variableImplementation.varID;
            this.observable = variableImplementation.observable;
            this.stateSpaceType = variableImplementation.stateSpaceType;
            this.attribute = variableImplementation.attribute;
            this.numberOfStates = variableImplementation.numberOfStates;
            this.distributionTypeEnum = variableImplementation.distributionTypeEnum;
        ...
        public boolean equals(Object o) {
            ...
            return this.getVarID()==var.getVarID()
                    // 07/06/2019 -> Añado el nombre para que el hashcode y el equals tengan el mismo contrato
                    && this.name.equals(var.getName());
        }
        ...
        @Override
        public int hashCode(){
            // 07/06/2019 -> Lo cambio porque el equals y el hashcode deberian tener el mismo contrato
            //return this.name.hashCode();
            return new HashCodeBuilder(17, 51)
                    .append(this.varID)
                    .append(this.name)
                    .hashCode();
        }
        ...
        public Variable deepCopy() {
            return new VariableImplementation(this);
        }
        ...
        @Override
        public void setDistributionTypeEnum(DistributionTypeEnum distributionTypeEnum) {
        ...
        public boolean isDiscrete() { return this.stateSpaceType.getStateSpaceTypeEnum() == StateSpaceTypeEnum.FINITE_SET; }

        public boolean isContinuous() { return this.stateSpaceType.getStateSpaceTypeEnum() == StateSpaceTypeEnum.REAL; }
    }
