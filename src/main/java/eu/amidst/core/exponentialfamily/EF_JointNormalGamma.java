 ...
 */
/*
    MyNote: La transformacion de los parametros a forma natural o momento se encuentra en Wikipedia:
...
public class EF_JointNormalGamma extends EF_UnivariateDistribution {
    ...
    /**
     * Creates a new EF_NormalGamma distribution with a default set of parameters. For an empirical parametrization
     * look at https://en.wikipedia.org/wiki/Normal-gamma_distribution#Interpretation_of_parameters
     * @param var1 a {@link Variable} object with a Normal-Gamma distribution type.
     */
    public EF_JointNormalGamma(Variable var1) {
        ...
        this.naturalParameters.set(EF_JointNormalGamma.MU_GAMMA, mean*precision);
        this.naturalParameters.set(EF_JointNormalGamma.MUSQUARE_GAMMA, -0.5*precision);
        this.naturalParameters.set(EF_JointNormalGamma.GAMMA, -beta-0.5*precision*mean*mean);
        this.naturalParameters.set(EF_JointNormalGamma.LOGGAMMA, alpha-0.5);

        /* MyNote: He eliminado la llamada a setNaturalParameters porque solo se hacia para llamar a estos 2 metodos */
        fixNumericalInstability();
        updateMomentFromNaturalParameters();
    }

    /**
     * Creates a new EF_NormalGamma distribution with a specific set of parameters. For an empirical parametrization
     * look at https://en.wikipedia.org/wiki/Normal-gamma_distribution#Interpretation_of_parameters
     * @param var a {@link Variable} object with a Normal-Gamma distribution type.
     */
    public EF_JointNormalGamma(Variable var, double[] parameters) {

        if (!var.isNormalGammaParameter())
            throw new IllegalArgumentException("The variable is not a Normal-Gamma parameter!");
        if(parameters.length != 4)
            throw new IllegalArgumentException("A Normal-Gamma distribution has 4 parameters. You passed an array with " + parameters.length);
