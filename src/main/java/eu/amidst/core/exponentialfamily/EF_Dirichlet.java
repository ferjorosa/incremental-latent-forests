...
/*
    ...
    Según Wikipedia la transformacion de parametro a parametro natural es directa
    (https://en.wikipedia.org/wiki/Exponential_family#Table_of_distributions)

    Sin embargo, en el codigo seguían quizas una version mas ortodoxa donde n = scale - 1
...
public class EF_Dirichlet extends EF_UnivariateDistribution {
    ...
    public EF_Dirichlet(Variable var1, double scale) {
        ...
        for (int i = 0; i < nOfStates; i++) {
            this.naturalParameters.set(i,scale);
        }
    ...
    /**
     * Creates a new EF_Dirichlet distribution for a given {@link Variable} object with a specific set of parameters (scale)
     * @param var
     * @param parameters
     */
    public EF_Dirichlet(Variable var, double[] parameters) {
        if (!var.isDirichletParameter())
            throw new IllegalArgumentException("The variable is not a Dirichlet parameter!");
        this.var=var;
        this.nOfStates = var.getNumberOfStates();
        this.naturalParameters = this.createZeroNaturalParameters();
        ...
        /* Its natural parameters transformation is straightforward */
        for (int i = 0; i < nOfStates; i++) {
            this.naturalParameters.set(i,parameters[i]);
        }
        fixNumericalInstability();
        updateMomentFromNaturalParameters();
    }
     ...
     */
    /*
     MyNote: Aqui es donde se devuelven los parametros de la posterior predictive de la multinomial, son los expected parameters
     porque como se puede ver en {https://en.wikipedia.org/wiki/Conjugate_prior}, esta aplicando la transformación de la
